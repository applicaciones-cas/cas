...
public class SectionController implements Initializable, ScreenInterface {
    ...
    private void initializeObject() {
        ...
        oParameters.Section().setRecordStatus("0123");
    }
        ...
        if (source instanceof Button) {
            ...
            switch (clickedButton.getId()) {
                ...
                case "btnNew":
                    ...
                    JSONObject poJSON = oParameters.Section().newRecord();
                ...
                case "btnBrowse":
                    ...
                    break;
                case "btnUpdate":
                    poJSON = oParameters.Section().updateRecord();
                    if ("error".equals((String) poJSON.get("result"))) {
                        ShowMessageFX.Information((String) poJSON.get("message"), "Computerized Acounting System", pxeModuleName);
                        break;
                    }
                    pnEditMode = oParameters.Section().getEditMode();
                    initButton(pnEditMode);
                    initTabAnchor();
                    break;
                case "btnCancel":
                    ...
                    break;
                case "btnSave":
                    oParameters.Section().getModel().setModifyingId(oApp.getUserID());
                    oParameters.Section().getModel().setModifiedDate(oApp.getServerDate());
                    JSONObject saveResult = oParameters.Section().saveRecord();
                    ...
                    switch (Status) {
                        case "0":
                            if (ShowMessageFX.YesNo(null, pxeModuleName, "Do you want to Activate this Parameter?") == true) {
                                poJsON = oParameters.Section().postTransaction();
                        ...
                        case "1":
                            if (ShowMessageFX.YesNo(null, pxeModuleName, "Do you want to Deactivate this Parameter?") == true) {
                                poJsON = oParameters.Section().voidTransaction();
                                ShowMessageFX.Information((String) poJsON.get("message"), "Computerized Accounting System", pxeModuleName);
                                loadRecord();
                            }
                            break;
...
//                            ShowMessageFX.Information((String) poJson.get("message"), "Computerized Acounting System", pxeModuleName);
//                        }
//                        oParameters.Section().getModel().setMeasureId(oParameters.Brand().getModel().getBrandId());
...
//                            ShowMessageFX.Information((String) poJson.get("message"), "Computerized Acounting System", pxeModuleName);
//                        }
//                        oParameters.Section().getModel().setSeriesId( oParameters.ModelSeries().getModel().getSeriesID());
...
//    }
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o, ov, nv) -> {
        ...
        if (!nv) {
            try {
                switch (lnIndex) {
                    case 1:
                        oParameters.Section().getModel().setSectionId(lsValue);
                        break;
                    case 2:
                        oParameters.Section().getModel().setSectionName(lsValue);
                        break;
                    default:
                        break;
                }
    ...
    private void loadRecord() {
        boolean lbActive = oParameters.Section().getModel().getRecordStatus() == "1";

        txtField01.setText(oParameters.Section().getModel().getSectionId());
        txtField02.setText(oParameters.Section().getModel().getSectionName());
        
        switch (oParameters.Section().getModel().getRecordStatus()) {
            case "1":
    ...
    void cbField01_Clicked(MouseEvent event) {
        if (cbField01.isSelected()) {
            oParameters.Section().getModel().setRecordStatus("1");
        } else {
            oParameters.Section().getModel().setRecordStatus("0");
        }
    }
