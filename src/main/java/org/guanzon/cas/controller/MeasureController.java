...
public class MeasureController implements Initializable, ScreenInterface {
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o, ov, nv) -> {
        ...
        if (!nv) {
            ...
            switch (lnIndex) {
                case 2:
                    poJSON = oTrans.getModel().setMeasurementName(lsValue);
                    if ("error".equals((String) poJSON.get("result"))) {
                        System.err.println((String) poJSON.get("message"));
                        ShowMessageFX.Information((String) poJSON.get("message"), "Computerized Acounting System", pxeModuleName);
    ...
    private void loadRecord() {
        ...
        psPrimary = oTrans.getModel().getMeasurementID();
        txtField01.setText(psPrimary);
        txtField02.setText(oTrans.getModel().getMeasurementName());
    ...
    private void loadTableDetail() {
        ...
        for (lnCtr = 0; lnCtr <= lnItem - 1; lnCtr++) {
            ListData.add(new ModelParameter(
                    (String) oTrans.getModelList().get(lnCtr).getMeasurementID(),
                    (String) oTrans.getModelList().get(lnCtr).getMeasurementName(),
                    "",
                    "",
                    ""));
