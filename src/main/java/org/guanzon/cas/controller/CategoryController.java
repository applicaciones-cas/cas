...
public class CategoryController implements Initializable, ScreenInterface {
    ...
    private void initializeObject() {
        ...
        oParameters.Category().setRecordStatus("0123");
    }
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o, ov, nv) -> {
        ...
        if (!nv) {
            try {
                switch (lnIndex) {
                    case 1:
                        oParameters.Category().getModel().setCategoryId(lsValue);
                        break;
                    case 2:
                        oParameters.Category().getModel().setDescription(lsValue);
                        break;
    ...
    private void loadRecord() {
        boolean lbActive = oParameters.Category().getModel().getRecordStatus() == "1";

        txtField01.setText(oParameters.Category().getModel().getCategoryId());
        txtField02.setText(oParameters.Category().getModel().getDescription());

        switch (oParameters.Category().getModel().getRecordStatus()) {
            case "1":
    ...
    void cbField01_Clicked(MouseEvent event) {
        if (cbField01.isSelected()) {
            oParameters.Category().getModel().setRecordStatus("1");
        } else {
            oParameters.Category().getModel().setRecordStatus("0");
        }
    }
