 ...
 * @author user
 */
public class PO_Quotation implements GTranDet{
    ...
    public PO_Quotation(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
        pbWthParent = fbWthParent;

        poModelMaster = new Model_PO_Quotation_Master(poGRider);
...
//        }
        ...
        if (poModelMaster.getEditMode() == EditMode.UPDATE) {
            ArrayList<Model_PO_Quotation_Detail> laOldTransaction = openTransactionDetail(poModelMaster.getTransactionNumber());

            if (laOldTransaction != null && !laOldTransaction.isEmpty()) {
                for (int lnCtr2 = lnCtr; lnCtr2 <= laOldTransaction.size() - 1; lnCtr2++) {
                    Model_PO_Quotation_Detail detail = laOldTransaction.get(lnCtr2);

                    lsSQL = "DELETE FROM " + detail.getTable()
                            + " WHERE sStockIDx = " + SQLUtil.toSQL(detail.getStockID())
                            + " AND nEntryNox = " + SQLUtil.toSQL(detail.getEntryNumber());
    ...
    @Override
    public Model_PO_Quotation_Detail getDetailModel(int fnRow) {
        ...
        switch (fsCol) {
            ...
            case "sDescript":
                ...
                if (poModelDetail.get(fnRow).getQuantity() > 0
                        && (!poModelDetail.get(fnRow).getDescription().isEmpty() || !poModelDetail.get(fnRow).getStockID().isEmpty())) {
                    AddModelDetail();
                }
            default:
...
//                }
    ...
    public ArrayList<Model_PO_Quotation_Detail> openTransactionDetail(String fsTransNo) {
        ArrayList<Model_PO_Quotation_Detail> loDetail = new ArrayList<>();

        try {
            Model_PO_Quotation_Detail placeholder = new Model_PO_Quotation_Detail(poGRider);
            String lsSQL = MiscUtil.addCondition(placeholder.makeSelectSQL(), "sTransNox = " + SQLUtil.toSQL(fsTransNo));
            ResultSet loRS = poGRider.executeQuery(lsSQL);

            while (loRS.next()) {
                Model_PO_Quotation_Detail detail = new Model_PO_Quotation_Detail(poGRider);
