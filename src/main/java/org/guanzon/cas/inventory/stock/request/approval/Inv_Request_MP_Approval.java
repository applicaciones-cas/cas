 ...
 * @author Unclejo
 */
public class Inv_Request_MP_Approval implements RequestApprovalController {
...
// Create a backup list to store deleted records temporarily
    ...
    public Inv_Request_MP_Approval(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
...
//                    if (poJSON != null) {
    ...
    public JSONObject searchTransaction(String fsColumn, String fsValue, boolean fbByCode) {
        ...
        String lsSQL = MiscUtil.addCondition(getSQL(), " a.sTransNox LIKE "
                + SQLUtil.toSQL(fsValue + "%") + " AND LEFT(a.sTransNox, 4) = " + SQLUtil.toSQL(poGRider.getBranchCode()) + " AND " + lsCondition + " AND f.sCategCd1 = '0002' GROUP BY a.sTransNox ASC");
...
//use for testing 
        ...
        try {
            ...
            Logger.getLogger(Inv_Request_MP_Approval.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//            String lsSQL = getSQL() + " GROUP BY a.sTransNox";
            ...
            String lsSQL = MiscUtil.addCondition(getSQL(), "f.sCategCd1 = '0002' AND LEFT(a.sTransNox,4) = " + SQLUtil.toSQL(poGRider.getBranchCode()) + "  GROUP BY a.sTransNox ASC");
            System.out.println(lsSQL);
            ResultSet loRS = poGRider.executeQuery(lsSQL);
            poMasterList = new ArrayList<>();
            while (loRS.next()) {
