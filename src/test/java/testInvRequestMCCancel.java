...
public class testInvRequestMCCancel {
    ...
    public void testNewTransaction() {
        ...
        loJSON = record.searchMaster(5, "M00124000022", true);
        if ("error".equals((String) loJSON.get("result"))) {
            Assert.fail((String) loJSON.get("message"));
        }   
        
//        Uncomment this code if setDetail on BrowseRequest is commented the code  
        loJSON = record.searchDetail(0, 3, "mc mc", true);
        if ("error".equals((String) loJSON.get("result"))) {
            Assert.fail((String) loJSON.get("message"));
        }
//        
...
//        comment this code if setDetail on BrowseRequest is commented the code 
        for(int lnCtr = 0; lnCtr < record.getDetailModel().size(); lnCtr++){
            Assert.assertEquals("M00124000022", record.getDetailModel().get(lnCtr).getOrderNumber());
            switch(lnCtr){
                    case 0:
                        Assert.assertEquals("M00124000118", record.getDetailModel().get(lnCtr).getStockID());
                        ...
                        Assert.assertEquals("mc mc", record.getDetailModel().get(lnCtr).getBarcode());
                        Assert.assertEquals("sample mc", record.getDetailModel().get(lnCtr).getDescription());
                        Assert.assertEquals("Motorcycle", record.getDetailModel().get(lnCtr).getCategoryName());
                        Assert.assertEquals("Scooter", record.getDetailModel().get(lnCtr).getCategoryName2());
